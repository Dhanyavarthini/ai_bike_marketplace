...
export async function processBikeImageWithAI(file) {
  try {
...
export async function addBike({ bikeData, images }) {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const { data, error } = await supabase.storage.from("bike-images").upload(filePath, imageBuffer, {
...
export async function updateBikeStatus(id, { status, featured }) {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
