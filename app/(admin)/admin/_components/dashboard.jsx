...
export function Dashboard({ initialData }) {
  ...
  const { bikes, testDrives } = initialData.data;
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{bikes.total}</div>
                <p className="text-xs text-muted-foreground">
                  {bikes.available} available, {bikes.sold} sold
                </p>
              </CardContent>
            </Card>
            <Card>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Bikes Sold</CardTitle>
                <DollarSign className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{bikes.sold}</div>
                <p className="text-xs text-muted-foreground">
                  {((bikes.sold / bikes.total) * 100).toFixed(1)}% of inventory
                </p>
              </CardContent>
            </Card>
          </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="font-medium text-sm mb-2">Bike Inventory</h3>
                    <div className="flex items-center">
                      <div className="w-full bg-gray-200 rounded-full h-2.5">
                        <div
                          className="bg-green-600 h-2.5 rounded-full"
                          style={{
                            width: `${(bikes.available / bikes.total) * 100}%`,
                          }}
                        ></div>
                      </div>
                      <span className="ml-2 text-sm">
                        {((bikes.available / bikes.total) * 100).toFixed(0)}%
                      </span>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-4 mt-6">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600 mt-1">Bikes Sold</p>
                  </div>
                  <div className="text-center">
                  ...
                  <div className="text-center">
                    <span className="text-3xl font-bold text-green-600">
                      {((bikes.available / (bikes.total || 1)) * 100).toFixed(0)}%
                    </span>
                    <p className="text-sm text-gray-600 mt-1">
