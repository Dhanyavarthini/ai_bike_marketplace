...
import { getBikes } from "@/actions/bike-listing";
import BikeListingsLoading from "./bike-listing-to-loading";
...
export function BikeListings() {
  ...
  const { loading, fn: fetchBikes, data: result, error } = useFetch(getBikes);
  ...
  const { data: bikes, pagination } = result;
    ...
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center text-center p-8 border rounded-lg bg-gray-50">
        ...
        <p className="text-gray-500 mb-6 max-w-md">
          We couldn't find any bikes matching your search criteria. Try adjusting
          your filters or search term.
        </p>
        <Button variant="outline" asChild>
          <Link href="/bikes">Clear all filters</Link>
        </Button>
      </div>
    );
  }
